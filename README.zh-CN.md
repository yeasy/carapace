...
- **v0.10**（当前）— 99 条 ExecGuard 模式（含 shell 归一化）、47 条 PathGuard 路径、Dashboard API 认证、SSRF 编码检测、数据外泄加固、1679 个测试
