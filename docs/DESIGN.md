...
**免费（开源 - MIT）**：
- 核心规则引擎（exec-guard、path-guard、network-guard）
...
**Pro（付费许可，$15/用户/月）**：
- Dashboard Web UI（事件时间线、skill 信任评分可视化）
- SIEM 集成（Splunk、ELK、Datadog 连接器）
- 团队集中式策略管理
- 高级 ML 异常检测
- 邮件/短信告警渠道
- 优先支持
...
假设：OpenClaw 有约 50 万活跃用户（基于 GitHub star 增长趋势）
- 第 1 年：聚焦免费层采用，目标 5K 安装量（1% 关注安全的用户）
- 第 2 年：500 个 Pro 订阅 = $90K ARR
- 第 3 年：2K Pro + 5 个 Enterprise = $500K ARR
...
- 所有工具调用被记录以构建初始基线
- 不触发异常告警（误报率太高）
- 5 个会话后：基线"冻结"，BaselineGuard 激活

**预热阶段（第 6 个会话起）**：
- BaselineGuard 将每次工具调用与 skill 画像对比
